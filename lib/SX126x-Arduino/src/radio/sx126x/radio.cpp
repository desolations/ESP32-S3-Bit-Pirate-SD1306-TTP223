...
static bool RadioTimeoutTimersInitialized = false;
...
void RadioInit(RadioEvents_t *events)
{
	...
	// Initialize the timeout timers once, then reuse them across repeated
	// LoRaService configure/release cycles. The ESP32 timer backend has a fixed
	// pool and TimerInit() does not release previously assigned slots.
	TxTimeoutTimer.oneShot = true;
	RxTimeoutTimer.oneShot = true;
	if (!RadioTimeoutTimersInitialized)
	{
		TimerInit(&TxTimeoutTimer, RadioOnTxTimeoutIrq);
		TimerInit(&RxTimeoutTimer, RadioOnRxTimeoutIrq);
		RadioTimeoutTimersInitialized = true;
	}
	else
	{
