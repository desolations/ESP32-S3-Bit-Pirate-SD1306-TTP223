...
Ssd1306Config CustomBoard::createDisplayConfig() {
    Ssd1306Config config;

    config.pinSda = CUSTOM_DISPLAY_I2C_SDA;
    config.pinScl = CUSTOM_DISPLAY_I2C_SCL;
    config.address = CUSTOM_DISPLAY_I2C_ADDRESS;
    config.flip = CUSTOM_DISPLAY_FLIP;
...
#if defined(CUSTOM_DISPLAY_DRIVER_ST7789_SPI) || defined(CUSTOM_DISPLAY_DRIVER_ST7789_PARALLEL) || defined(CUSTOM_DISPLAY_DRIVER_SSD1306)
    deviceView.logo();
    delay(900);
    deviceView.clear();
#endif
